...
- Running Codex via `cco --safe` (macOS Seatbelt sandbox)
...
but NOT parents. So even with `(allow file-read* (subpath "$HOME/.Codex"))`, `lstat("$HOME")`
...
```bash
src="$HOME/.Codex/statusline-command.ts"
cached="/tmp/Codex-statusline-${UID}.ts"
if [[ ! -f "$cached" ]] || [[ "$src" -nt "$cached" ]]; then
  cat "$src" > "$cached" 2>/dev/null || { echo "Codex"; exit 0; }
fi
...
Codex  # should start without EPERM errors
