...
- To load the whole file in one session: `codex -c project_doc_max_bytes=200000`. Setting it permanently means editing `~/.codex/config.toml`, which this repository does not manage yet (#312 / #315). The limit applies to this **project** file only — `~/.codex/AGENTS.md` is delivered in full at any size (measured 2026-09-14 as a Contrast Pair: the same 53 KB file truncates as a project doc and does not truncate as the global doc).
...
- **`~/.claude/` is a deploy target, not your configuration.** Paths under `~/.claude/` are described below because this repository generates them from `dot_claude/`. They are not where your own settings live, and editing them does not change your behavior. The one exception is `~/.claude/rules/**`, which your *global* instructions ask you to read explicitly — that is a shared rule library, not Claude Code configuration.
...
- **Global instructions are out of scope for this file.** `AGENTS.md` here covers this repository only. Your user-global instructions come from `~/.codex/AGENTS.md`, which this repository now generates from `harness/modules/global/` plus `harness/manifest.json` (#311) — regenerate with `just harness-sync-global`, and never edit the deployed file, since the next sync overwrites it. Cursor receives the same global policy through `.cursor/rules/dotfiles.mdc` instead, because it has no writable global surface.
...
just test-harness-global-instructions # Smoke test the global instruction sync against a fixture root
just check-global-instructions # Drift check for ~/.claude/CLAUDE.md and ~/.codex/AGENTS.md (local only, not in `lint`)
...
- **`~/.claude/CLAUDE.md` and `~/.codex/AGENTS.md` are generated too, and chezmoi does not own them** — Their Source is `harness/modules/global/` plus `harness/manifest.json` (#311); regenerate with `just harness-sync-global`, check with `just check-global-instructions` (local only — the global Targets are not committed, so CI cannot compare them; `just test-harness-global-instructions` is the CI-side Source check). `chezmoi source-path` does not resolve either file, and `chezmoi apply` no longer creates `~/.claude/CLAUDE.md` — **on a new machine run `just harness-sync-global` once** until #324 wires global sync into apply. The SessionStart harness briefing names whichever of the two is missing, so the gap announces itself from any repository rather than only this one. It renders from the **current worktree**, the exact opposite of `chezmoi apply` (which deploys from `main`) — running it on a feature branch puts unmerged text into every project's global instructions, so run it only from a merged tree.
- **Never edit deployed targets directly** — Always edit the chezmoi source file (under `~/.local/share/chezmoi/`), never the deployed target (under `~/`). For example, edit `dot_claude/scripts/executable_harness-briefing.sh`, not `~/.claude/scripts/harness-briefing.sh`. Changes to deployed targets are overwritten on next `chezmoi apply` and are not version-controlled. Use `chezmoi source-path <target>` to find the source file for any managed target — except the harness-owned global instructions above.
