...
- **MCP stdio servers do not pass through zsh wrapper functions** — MCP entries (declared in `dot_apm/apm.yml`, written by APM into `~/.claude.json`'s `mcpServers` and `~/.codex/config.toml`'s `[mcp_servers.*]`) are exec'd directly, so a `foo()` wrapper defined in `dot_config/zsh/` never runs for a server whose `command` is `foo`. Measured on the since-removed `codex` MCP server: the `codex()` wrapper's nested-sandbox handling did not apply, and nothing was needed anyway (nesting does not break the stdio server — it connected in 89 ms inside nono, and `codex mcp-server` accepts no `--sandbox` flag at all; only *interactive* codex needs `--sandbox danger-full-access`). For a server that *does* need a wrapper's behaviour, the flag must go in `args` or the command must point at a shim script.
- **APM's MCP prune is keyed by name in the lock, not by what APM wrote** — dropping a server from `dot_apm/apm.yml` deletes that name from **every** active target, hand-written entries included (verified: `[mcp_servers.codex]` with `enabled = false` was removed from `~/.codex/config.toml`). Names absent from the lock (`node_repl`, `[projects.*]`) are untouched. Never hand-add a server under a name `apm.yml` also uses. Detail: `docs/adr/0006-apm-owns-only-the-mcp-servers-table-of-codex-config.md`.
- **APM never updates an existing MCP entry in place** — change a server's `command`/`args`/`url` in `apm.yml` and `apm install` prints `already configured`, leaving the stale definition with no error. Workaround: drop it, install (prunes), restore, install again. Tracked in #349.
