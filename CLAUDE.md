...
- **Inline hook commands: keep simple or use jq** — `settings.json.tmpl` のインライン `bash -c` hook コマンドでは、JSON エスケープ（`\"`）とシェルクォーティングの二重レイヤーが極めて混乱しやすい。複雑な grep/sed パターンは避け、`jq`（既に依存）を使うか、外部スクリプトファイルに切り出す。
- **Hook scripts: set one-shot flags after guards, not before** — `/tmp` フラグファイルで「セッション中1回だけ実行」を実現する場合、フラグの `touch` はコンテキストガード（ディレクトリ除外、git チェック等）の**後**に配置する。ガード前に touch すると、非該当コンテキスト（`$HOME` 等）でフラグが消費され、後でプロジェクトに移動してもスキップされるサイレント不具合になる。
