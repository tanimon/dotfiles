...
- **`.chezmoiignore` bare filenames block `dot_` sources** — `.chezmoiignore` evaluates target paths. Adding `.gitignore` blocks `dot_gitignore` → `~/.gitignore` deployment. Repo-root files without chezmoi prefixes (`dot_`, `private_`, etc.) are already invisible to chezmoi and do not need `.chezmoiignore` entries. Only add entries for files that use chezmoi naming conventions.
