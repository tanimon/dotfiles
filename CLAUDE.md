...
- **`~/.claude/settings.json` is fought over by chezmoi and APM, not just the nono pack** — the old hand-maintained `enabledPlugins`/`extraKnownMarketplaces` blocks (kept in sync with the nono pack's install-time write) were removed entirely in Task 8 of the APM migration; APM does not use Claude Code's native plugin-marketplace registry. Instead, `apm install --global --target claude` deploys plugin Skill/Command/Hook content directly under `~/.claude/` and merges hook definitions straight into `settings.json`'s `hooks` key — confirmed real-machine via `jq '.hooks.SessionStart'` showing APM-authored entries (see the Task 6/7 reports under `.superpowers/sdd/2026-08-02-apm-skill-mcp-management/`). `run_after_apm-install.sh.tmpl` re-runs this install on every `chezmoi apply` (not hash-gated) precisely because that merged `hooks` content would otherwise be silently wiped by the next template render, which still fully owns `settings.json`. Verify with `jq '.hooks' ~/.claude/settings.json` after apply that APM-authored hooks (e.g. superpowers' `SessionStart`) are present.
