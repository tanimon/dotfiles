...
- **The nono pack writes `enabledPlugins` into `~/.claude/settings.json`** — chezmoi fully owns `settings.json.tmpl`, and the template no longer hand-maintains this key (Task 8 of the APM migration removed it along with `extraKnownMarketplaces`). `run_after_apm-install.sh.tmpl` restores `enabledPlugins` by running `apm install --global --target claude` on every `chezmoi apply`, *after* the template has overwritten `settings.json` — this is why that script is not hash-gated. Verify with `jq '.enabledPlugins' ~/.claude/settings.json` after apply to confirm APM repopulated it.
