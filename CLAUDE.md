...
- **(historical, Task 8 of the APM migration, 2026-08-03 — superseded 2026-08-10) `~/.claude/settings.json` was fought over by chezmoi and APM, not just the nono pack** — the old hand-maintained `enabledPlugins`/`extraKnownMarketplaces` blocks (kept in sync with the nono pack's install-time write) were removed entirely in Task 8 of the APM migration; APM does not use Claude Code's native plugin-marketplace registry. Instead, `apm install --global --target claude` deployed plugin Skill/Command/Hook content directly under `~/.claude/` and wrote hook definitions straight into `settings.json`'s `hooks` key (discovered during Task 6 implementation; see `docs/superpowers/specs/2026-08-02-apm-skill-mcp-management-design.md`, "実装時の訂正" item 2), requiring `run_after_apm-install.sh.tmpl` to re-run this install on every `chezmoi apply` (not hash-gated) so that merged `hooks` content wasn't silently wiped by the next template render. Skill/plugin management has since reverted to native `enabledPlugins`/`extraKnownMarketplaces` (see "Skill/plugin management via native Claude Code marketplace" above), so this specific hooks-conflict no longer applies — `dependencies.apm` is empty, `apm install` only touches MCP servers (in `~/.claude.json`, not `settings.json`), and the install script is back to a plain hash-gated `run_onchange_`. Kept here as a record of why `run_after_` vs `run_onchange_` mattered; see [run-after-vs-run-onchange-for-shared-config-ownership.md](docs/solutions/architecture-patterns/run-after-vs-run-onchange-for-shared-config-ownership.md) for the general lesson.
