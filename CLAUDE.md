...
- **Git commit signing** — 対話ターミナルの `~/.gitconfig` は 1Password SSH agent(`op-ssh-sign`)で署名するので、1Password が動いていないと commit が失敗する。**Claude Code の git は別**: `dot_config/git/claude-code.inc`(`GIT_CONFIG_GLOBAL` 経由)が `user.signingkey` を `~/.config/git/signing/ai-agent`(`run_once_generate-ai-agent-signing-key.sh.tmpl` が生成するパスフレーズ無しのローカル鍵)、`gpg.ssh.program` を素の `ssh-keygen` に上書きするため、1Password のロック状態に依存せず人間不在でも署名が通る。鍵は `~/.ssh` 外に置く(両サンドボックスが `~/.ssh` を読み取り拒否するため)。公開鍵を GitHub に **signing key として登録**するのは人手ステップで、未登録だと Unverified になり `required_signatures` ルールのあるリポジトリ(このリポジトリを含む)ではマージできない。詳細は `claude-code.inc` のコメントと `dot_config/nono/CLAUDE.md` の署名段落。
