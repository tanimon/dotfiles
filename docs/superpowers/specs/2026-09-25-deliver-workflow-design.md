...
3. **レビューループ**(修正ラウンドの上限 `maxReviewRounds`、既定 3。修正のたびに再レビューするので、レビューは最大で上限+1回。上限はループに入るたびに数え直す)。各ラウンドは次の順で進む。
   ...
   10. いずれかのラウンドで結果を返さなかったレビュアーがいれば、報告の先頭で「ラウンド N: <レビュアー>」の形で警告する(そのラウンドの確認が不完全なため)。
...
5. **公開**: push して draft PR を作る。base との差分コミットが無い(何も実装せずに止まった)場合は PR を作らない。本文は下記「報告」の構成で、コードが組み立てる。同じ内容を ledger に保存し、Workflow の `return` でセッションにも返す。
...
`budget.total` が設定されている場合は、実装タスク・レビューラウンド・動作確認の再試行それぞれの開始前に残量を確認し、`BUDGET_FLOOR` を割っていれば停止理由 `budget` で公開の段へ進む(上限に達すると以後の `agent()` はすべて throw し、公開すらできなくなるため)。既定の上限値は、試作を数回実行して計測してから決める。試作の段階では、統計に `budget.spent()` を記録するだけにとどめる。
