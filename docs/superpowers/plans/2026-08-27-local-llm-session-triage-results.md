...
| モデル | 低シグナル層 recall | 低シグナル層 偽陽性 | 高シグナル層 recall | ms/件(gold) | 238 件の推定時間 |
|---|---|---|---|---|---|
| qwen3:8b (v2) | **100% (3/3)** | 8 / 11 | 100% (6/6) | 101,065 | 401 分 |
| qwen3:14b (v2) | 100% (3/3) | 7 / 11 | 100% (6/6) | 131,554 | 522 分 |
...
- pending.jsonl からの処理済み 3 件の削除(SKILL.md の作法: `grep -vF` → temp → `mv`)は、**auto mode の permission classifier にブロックされ未実施**。次回の /harness-reflect 実行時または手動での削除が必要
