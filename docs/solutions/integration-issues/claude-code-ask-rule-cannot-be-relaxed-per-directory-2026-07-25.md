...
**`ask` は床（floor）であって既定値（default）ではない。** どのスコープからも床を上げる（緩める）
ことはできるが、床を下げる（さらに緩める）ことはできない。`allow` ルールも `PreToolUse` フックの
`"allow"` 判定も、この床より**下**に位置する — つまりどちらも既存の `ask`/`deny` を上書きする力を
持たない。

したがって位置依存の権限制御は、「制限の強い床を選んでそこから例外を切り出す」形では実現できない。
実現できるのは「許容的な床を選び、その上に制限を積み増す」形だけである。今回 (A) が失敗したのは
まさにこの逆方向 — 既存の `ask` という床の上から緩和を試みたから — であり、(B) は最初から床の位置
そのものをコマンドの性質に応じて設計し直すことで成立している。
