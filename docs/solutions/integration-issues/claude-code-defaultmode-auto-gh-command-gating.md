...
**2026-07-25 追記:** 上記の「`gh` は ask ゲート対象」は、当時 `ask` に列挙されていた更新系 verb
全体を指した記述である。現在は一部が `allow` に移っており、`gh` 全体には当てはまらない。リスク
Tier モデル（`docs/superpowers/specs/2026-07-25-permission-tier-model-design.md`）に基づき
`gh pr comment` / `gh issue comment` を `allow` へ移したため、この 2 つは「サンドボックス外」かつ
「ゲートなし」になる。`gh api` も従来どおり未列挙で自動承認のままである（#225）。`ask` ゲートが
唯一の統制であるという性質は、`ask` に明示列挙されている更新系 verb に限って成立する。
