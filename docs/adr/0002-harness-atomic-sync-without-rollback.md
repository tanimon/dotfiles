...
- staging は `$TMPDIR` 配下で、親シェルが `mktemp -d` してから `main` を subshell で実行し、成功・失敗のどちらでも親が削除する。INT / TERM / HUP は親が subshell に転送してから staging を削除し、subshell は置換中の書きかけの一時ファイルを消して exit 130 する(中断で先に置換済みの Target は巻き戻さない)。EXIT trap で削除しないのは、bash 3.2 では EXIT trap があると `set -u` 違反の終了コードが 0 に潰れるため(ADR 0003)。
